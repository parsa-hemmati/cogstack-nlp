...
async def rate_limit_search(
    ...
    Limits searches to {limit} requests per {window} seconds per user.
    ...
    Example:
        @router.post("/search", dependencies=[Depends(rate_limit_search)])
        async def search(...):
            pass
    """
    try:
...
def create_rate_limiter(limit: int, window: int = 60) -> Callable:
    ...
    async def rate_limiter(
        ...
        await rate_limit_search(request, current_user, limit=limit, window=window)
